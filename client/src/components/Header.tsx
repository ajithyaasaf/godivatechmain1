      ...
      >
        ...
        <AnimatePresence>
          {isMenuOpen && (
            ...
            >
              <motion.div 
                className="absolute top-[61px] right-0 bottom-0 w-[80%] max-w-sm bg-white shadow-2xl flex flex-col rounded-l-2xl overflow-hidden"
                initial={{ x: "100%" }}
                animate={{ x: 0 }}
                exit={{ x: "100%" }}
                transition={{ type: "spring", damping: 25, stiffness: 300 }}
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto py-2">
                  {mainNavItems.map((item, index) => (
                    <div 
                      key={item.path} 
                      className={`mb-1 ${index !== mainNavItems.length - 1 ? 'border-b border-neutral-100' : ''}`}
                    >
                      {/* Main menu item */}
                      <Link
                        href={item.path}
                        className={`flex justify-between items-center px-5 py-3 font-medium transition-colors ${
                          isActive(item.path)
                            ? "text-primary bg-primary/5"
                      ...
                      >
                        <div className="flex items-center gap-3">
                          {item.label === "Home" && <Box className="h-4.5 w-4.5 text-neutral-500" />}
                          {item.label === "Services" && <Layers className="h-4.5 w-4.5 text-neutral-500" />}
                          {item.label === "Portfolio" && <BrainCircuit className="h-4.5 w-4.5 text-neutral-500" />}
                          {item.label === "About" && <CircleUser className="h-4.5 w-4.5 text-neutral-500" />}
                          {item.label === "Blog" && <Braces className="h-4.5 w-4.5 text-neutral-500" />}
                          {item.label === "Contact" && <Palette className="h-4.5 w-4.5 text-neutral-500" />}
                          <span>{item.label}</span>
                        </div>
                        {item.hasMegaMenu && (
                          <ChevronDown className={`h-5 w-5 transition-transform ${isMegaMenuOpen ? 'rotate-180' : ''}`} />
                        )}
                      </Link>
                      
                      {/* Mobile submenu for services */}
                      {item.hasMegaMenu && (
                        <div className="bg-neutral-50 py-2 px-3">
                          <div className="space-y-1 pl-3 ml-3">
                            {serviceItems.map((service, idx) => (
                              <Link 
                                key={idx}
                                href={service.path}
                                className="flex items-center px-3 py-2.5 rounded-lg text-neutral-700 hover:bg-white hover:text-primary transition-colors"
                                onClick={() => setIsMenuOpen(false)}
                              >
                                <div className={`flex items-center justify-center h-8 w-8 rounded-full ${service.color} bg-opacity-10 mr-3`}>
                ...
                {/* Footer with CTA button and social links */}
                <div className="p-5 border-t border-neutral-100 bg-neutral-50">
                  <Button
                    asChild
                    variant="default"
                    className="w-full bg-primary hover:bg-primary/90 rounded-full py-2.5 shadow-md"
                    onClick={() => setIsMenuOpen(false)}
                  >
                  ...
                  <div className="mt-5 pt-3 border-t border-neutral-200">
                    <p className="text-xs text-neutral-500 mb-3 text-center">Connect with us</p>
                    <div className="flex justify-center space-x-5 text-neutral-400">
