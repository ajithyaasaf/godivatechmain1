  ...
  return (
    <>
      ...
      >
        ...
        {/* Beautiful fullscreen mobile menu overlay */}
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div 
            ...
            >
                  ...
                  {mainNavItems.map((item, index) => (
                    ...
                    >
                      {!item.hasMegaMenu ? (
                        ...
                        >
                          <div className="flex items-center">
                            <span className="flex items-center justify-center w-10 h-10 rounded-full bg-white/10 mr-4">
                              {item.label === "Home" && <Box className="h-5 w-5" />}
                              {item.label === "Portfolio" && <BrainCircuit className="h-5 w-5" />}
                              {item.label === "About" && <CircleUser className="h-5 w-5" />}
                              {item.label === "Blog" && <Braces className="h-5 w-5" />}
                              {item.label === "Contact" && <Palette className="h-5 w-5" />}
                            </span>
                            {item.label}
                          </div>
                        </Link>
                        ...
                        <div>
                          <div className="flex items-center py-3 text-xl font-medium">
                            <span className="flex items-center justify-center w-10 h-10 rounded-full bg-white/10 mr-4">
                          ...
                          <div className="grid grid-cols-2 gap-3 mt-4 pl-14">
                            {serviceItems.map((service, idx) => (
                              <Link 
                                key={idx}
                                href={service.path}
                                className="bg-white/10 hover:bg-white/20 rounded-xl p-3 transition-colors group"
                                onClick={() => setIsMenuOpen(false)}
                              >
                                <div className={`${service.color} bg-opacity-20 h-10 w-10 rounded-full flex items-center justify-center mb-2`}>
                                  <service.icon className={`h-5 w-5 ${service.color}`} />
                                </div>
                                <span className="text-sm font-medium block group-hover:text-white">{service.title}</span>
                              </Link>
                            ))}
                          </div>
                        </div>
              ...
              {/* Footer area with CTA and socials */}
              <motion.div 
              ...
              >
                <Button
                  asChild
                  className="w-full bg-white text-primary hover:bg-white/90 rounded-full py-6 shadow-lg font-medium"
                  onClick={() => setIsMenuOpen(false)}
                >
